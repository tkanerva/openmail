...
class Options(usage.Options, strcred.AuthOptionMixin):
    ...
    def __init__(self):
        ...
        self.service = MyMailService()  # a customized MailService class
    ...
    def opt_kvdirdomain(self, domain):
        """Generate an SMTP/POP3 virtual domain, using KVDomain.
        ...
        try:
            ...
            raise usage.UsageError("Argument to --kvdirdomain must be of the form 'name=path'")

        self.last_domain = kvdomain.KVDomain(self.service, os.path.abspath(path))
        print 'DEBUG:', self.last_domain.getCredentialsCheckers()
        self.service.addDomain(name, self.last_domain)
    opt_d = opt_kvdirdomain
...
class MyMailService(mail.MailService):
    """ provide a custom IMailDelivery """
    def __init__(self):
        print('My custom MailService starting...')
        mail.MailService.__init__(self)
        self.authed = False  # to be checked in validateTo

    # override this
    def requestAvatar(self, avatarId, mind, *interfaces):
        if smtp.IMessageDelivery in interfaces:
            log.msg('Creating MyESMTPDelivery')
            a = MyESMTPDomainDelivery(self, avatarId)
            self.authed = True  # at this point, we are successfully authed
            return smtp.IMessageDelivery, a, lambda: None
        raise NotImplementedError()

class MyESMTPDomainDelivery(protocols.DomainDeliveryBase):
    """ custom Delivery class that does not relay unless authed."""
    protocolName = 'esmtp'
    ...
    def validateTo(self, user):
        ...
        if self.user and self.service.queue:
            ...
            log.msg('Debug:\n %s' % dir(self.service))
            if d is None and not self.service.authed:
                raise smtp.SMTPDeliveryError(550, "We do not relay unless you have an account.")
            elif d is None and not self.orig_is_local:  # relay foreign addresses
                log.msg('Debug: d is None and not self.orig_is_local')
                d = relay.DomainQueuer(self.service, True)
            elif d is None and self.orig_is_local:  # we know the sender, lets relay
                d = relay.DomainQueuer(self.service, True)
        else:
            d = self.service.domains[user.dest.domain]  # delivery to local domain
